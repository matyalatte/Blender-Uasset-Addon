...
class SkeletalLODSection(LODSection):
    ...
    def copy(self):
        return SkeletalLODSection(self.version, self.material_id, self.first_ib_id, self.face_num, self.unk, \
...
class SkeletalLODSection5(LODSection):
    ...
    def read(f, version):
        ...
        material_id=read_uint16(f)
        first_ib_id=read_uint32(f)
        face_num = read_uint64(f)
        unk = read_uint8(f)
        cast_shadow = read_uint32(f)
        if version>='5.0':
            ray_tracing = read_uint32(f)
        else:
            ray_tracing = None
        first_vertex_id=read_uint64(f)
        vertex_group=read_uint16_array(f)
        vertex_num=read_uint32(f)
        max_bone_influences=read_uint32(f)
        check(f.read(2), b'\xff\xff') #CorrespondClothAssetIndex?
        read_null_array(f, 4) #AssetGuid for FClothingSectionData?
        check(read_int32(f), -1) #AssetLodIndex for FClothingSectionData?
        unk_ids = read_uint32_array(f)
    ...
    def copy(self):
        return SkeletalLODSection5(self.version, self.material_id, self.first_ib_id, self.face_num, self.unk,\
                    self.first_vertex_id, self.vertex_group, self.vertex_num, self.max_bone_influences, \
    ...
    def write(f, section):
        ...
        write_uint16(f, section.material_id)
        write_uint32(f, section.first_ib_id)
        write_uint64(f, section.face_num)
        write_uint8(f, section.unk)
        write_uint32(f, section.cast_shadow)
        if section.version>='5.0':
            write_uint32(f, section.ray_tracing)
        write_uint64(f, section.first_vertex_id)
        write_uint16_array(f, section.vertex_group, with_length=True)
        write_uint32(f, section.vertex_num)
        write_uint32(f, section.max_bone_influences)
        ...
        write_null_array(f, 4)
        write_int32(f, -1)
        write_uint32_array(f, section.unk_ids, with_length=True)
        write_uint32(f, section.vertex_num)
